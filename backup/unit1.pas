...
begin
    ...
    ECVolSlider1.Position :=  TooLoud;
    Volume:=  ECVolSlider1.Position;
    LazSerial1.WriteData('SET MSCLVL '+(FloatToStr(Volume))+#13);
end;

procedure TForm1.ECVolSlider1MouseDown(Sender: TObject; Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
begin
  Form1.ECVolSlider1.GrooveColor:=clMaroon;
end;
...
procedure TForm1.LazSerial1RxData(Sender: TObject);
 ...
 begin
 InputString:= LazSerial1.ReadData;

if LeftStr(InputString,8)='MSCLVL -' then   //- it's a volume number
     begin
        Volume:=StrToFloat(copy(InputString, 8, 3));
        ECVolSlider1.Position:= Volume;
     end
else
    if LeftStr(InputString,6)='SELECT' then     // it's a source letter
     begin
 ...
 if   LeftStr(InputString,7)='Command' then     // it's an acknowledgement
    begin
      Delay(10);
...
with Sender as TMenuItem do
  ...
  Case Name of
   ...
   Form1.LazSerial1.WriteData('SET SELECT '+Choice+#13);
   Delay(25);
   LazSerial1.WriteData('GET SELECT '+#13);
   Delay (10);
  end;
end;
